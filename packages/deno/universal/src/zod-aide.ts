...
>(original: T) {
  return original instanceof z.ZodOptional ||
    original instanceof z.ZodNullable ||
    original instanceof z.ZodDefault
...
>(baggageProperty: BaggageProp) {
  ...
  // deno-fmt-ignore
  const isProxyQueryProp = `is${titleCase(
    String(baggageProperty),
  )}Proxy` as const;
  ...
  const zodTypeBaggageProxy = <ZodType extends z.ZodTypeAny>(
    ...
    return new Proxy<ZodType>(zodType, {
...
export function zodSchemaProxy<
  ...
  >,
>(schema: z.Schema<Proxy, ZodTypeDef, In>, methods: Methods) {
  type DropFirstTuple<T extends Any[]> = T extends [Any, ...infer R]
    ? R
    : never;
